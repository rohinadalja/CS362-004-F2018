...
int handleSmithyCardEffect(int currentPlayer, struct gameState *state, int handPos)
{
  int i;
  //+3 Cards
  for (i = 0; i < 2; i++)
...
int handleCouncilRoomCardEffect(int currentPlayer, struct gameState *state, int handPos)
{
  int i;
...
int handleFeastCardEffect(struct gameState *state, int currentPlayer, int tempHand[], int choice1)
{
  ...
  while (x == 1)
  {  // Buy one card
    if (supplyCount(choice1, state) <= 0 && DEBUG)
    {
...
int handleGreatHallCardEffect(int currentPlayer, struct gameState *state, int handPos)
{
    //+1 Card
    drawCard(currentPlayer, state);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case council_room:
      return handleCouncilRoomCardEffect(currentPlayer, state, handPos);
			
    case feast:
      return handleFeastCardEffect(state, currentPlayer, temphand, choice1);
    ...
    case great_hall:
      return handleGreatHallCardEffect(currentPlayer, state, handPos);
